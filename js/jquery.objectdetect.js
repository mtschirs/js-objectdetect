(function($) {
	...
	var detect = function(options, callback, detectorFn) {
		...
		var rects = detectorFn(sat, rsat, ssat, undefined, canvasWidth, canvasHeight, options.classifier, options.scaleFactor, options.scaleMin);
	...
	var methods = {
		...
		all: function(options, callback) {
			...
			return this.each(function() {
				detect.apply(this, [options, callback, objectdetect.detectMultiScale]);
			});
		},
		
		finest: function(options, callback) {
			options = $.extend({}, $.fn.objectdetect.options, options);
			if (!options.classifier) throw "jQuery.objectdetect cannot operate without specified object classifier";
			
			return this.each(function() {
				detect.apply(this, [options, callback, objectdetect.detectFinestScale]);
	    	});
		}
	};
