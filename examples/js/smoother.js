Smoother = function(alpha, initPos) {
	...
	var sp = initPos, //[initPos[0], initPos[1], initPos[2], initPos[3], initPos[4]],
		sp2 = sp,
		sl = sp.length;
	...
	this.smooth = function(pos) {
		positions = pos;//[pos[0], pos[1], pos[2], pos[3], pos[4]];
		
		// update
		for (var i = 0;i < sl;i++) {
			sp[i] = alpha[i] * positions[i] + (1-alpha) * sp[i];
			sp2[i] = alpha[i] * sp[i] + (1-alpha) * sp2[i];
		}
